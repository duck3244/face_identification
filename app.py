...
from config import (
    ...
    DEFAULT_THRESHOLD, DEFAULT_TOP_K, DATABASE_FILE, DATABASE_DIR, get_logger
)
...
def fn_recognize(img_path: str, threshold: float, top_k: int):
    if img_path is None:
        return None, ""
    top_k = int(top_k)
    results = state.system.recognize_face(img_path, threshold, top_k)
    annotated = visualize_recognition(img_path, results)
    if results:
        rows = [[identity, f"{score:.4f}"] for identity, score in results]
    else:
        rows = [["일치하는 얼굴 없음", "-"]]
    table_html = _make_html_table(["인물", "유사도"], rows)
...
def fn_detect(img_path: str):
    ...
    face_info = state.system.detect_face(img_path)
    ...
    extracted = state.system.extract_face(img_path)
    extracted_img = None
