...
from validators import validate_threshold, validate_top_k, validate_identity
...
class FaceDatabase:
    ...
    def search(
        ...
        query_array = np.atleast_2d(np.asarray(query_embedding, dtype='float32'))
    ...
    def _load_and_migrate_pkl(self, pkl_path: str) -> bool:
        """레거시 pkl 파일을 로드하고 새 형식으로 마이그레이션 후 pkl 제거"""
        import pickle
        ...
        try:
            os.remove(pkl_path)
            logger.info(f"레거시 pkl 파일이 새 형식으로 마이그레이션되고 제거되었습니다: {pkl_path}")
        except OSError as e:
            logger.warning(f"pkl 파일 제거 실패 (수동 삭제 필요): {pkl_path} ({e})")
        return True
