...
class TestFaceDatabase:
    ...
    def test_add_face_rejects_empty_identity(self, tmp_image):
        db = FaceDatabase("VGG-Face", "cosine")
        with pytest.raises(ValueError):
            db.add_face(tmp_image, "")
        with pytest.raises(ValueError):
            db.add_face(tmp_image, "   ")

    def test_search_accepts_ndarray(self, sample_embedding):
        """ndarray로 query 넘겨도 차원 오류 없이 동작 (1.3 회귀 방지)"""
        db = FaceDatabase("VGG-Face", "cosine")
        db.embeddings = [sample_embedding]
        db.identities = ["test_person"]
        db.build_index()

        query = np.asarray(sample_embedding, dtype='float32')
        results = db.search(query, threshold=0.5, top_k=1)
        assert len(results) == 1
        assert results[0][0] == "test_person"
